...
public class RateTableDemo2 {
    ...
    public static void main(String[] args) {
        ...
        ExchangeRate lego = new ExchangeRate("legoVIPPoints",
                new BigDecimal(6.5));

        System.out.println(
                "Current rate of LEGO VIP points to USD: " + lego.rate());
        System.out.println(
        ...
        try {
            input = in.readLine();
            ExchangeRate newLego = new ExchangeRate("legoVIPPoints",
                    lego.multiplyRate(new BigDecimal(input)));
            System.out.println(
            ...
            if (newLego.isStable(lego, new BigDecimal(4.1))) {
                ...
                System.out.println("This inflation is unstable.");
            }

            RateTable t = new MapRateTable();
            t.populateRatesFromAPI(); // May not work since env file is uncommitted
            System.out.println("It could even has much as Venezuelan Bolívar, "
                    + newLego.convertAmount(t.getExchangeRate("VES")));
        } catch (IOException e) {
